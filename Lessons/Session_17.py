# Python Tuples: Tuples are used to store multiple items in a single variable. Tuple is one of 4 built-in data types in
# Python used to store collections of data, the other 3 are List, Set, and Dictionary, all with different qualities and
# usage.
...
# Ordered: When we say that tuples are ordered, it means that the items have a defined order, and that order will not
# change.
...
print(thistuple[2:5])
# Note: The search will start at index 2 (included) and end at index 5 (not included). The first item has index 0.

# By leaving out the start value, the range will start at the first item. This example returns the items from the
# beginning to, but NOT included, "kiwi":
thistuple = ("apple", "banana", "cherry", "orange", "kiwi", "melon", "mango")
print(thistuple[:4])
...
# Add Items: Since tuples are immutable, they do not have a built-in append() method, but there are other ways to add
# items to a tuple.
...
# 2. Add tuple to a tuple. You are allowed to add tuples to tuples, so if you want to add one item, (or many), create a
...
thistuple += y
print(thistuple)
# Note: When creating a tuple with only one item, remember to include a comma after the item, otherwise it will not be
...
# Using Asterisk*: If the number of variables is less than the number of values, you can add an * to the variable name
...
# Using a While Loop: You can loop through the tuple items by using a while loop.
...
# Parameter: value; Description: Required. The item to search for
