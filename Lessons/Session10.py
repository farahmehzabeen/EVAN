...
# The range() Function: To loop through a set of code a specified number of times, we can use the range() function, the
# range() function returns a sequence of numbers, starting from 0 by default, and increments by 1 (by default), and ends
...
# Nested Loops: A nested loop is a loop inside a loop. The "inner loop" will be executed one time for each iteration of
# the "outer loop":
...
# The pass Statement: for loops cannot be empty, but if for some reason there is a for loop with no content, put in the
